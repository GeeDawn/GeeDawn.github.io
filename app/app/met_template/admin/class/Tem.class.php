...
class Tem
{
    ...
    public function updateUi($skin_name = '', $ui = array())
    {
        ...
        foreach ($json['config'] as $key => $val) {
            $query = "SELECT * FROM {$_M['table']['ui_config']} WHERE uip_key = '{$val['uip_key']}' AND parent_name = '{$val['parent_name']}' AND ui_name = '{$val['ui_name']}' AND skin_name = '{$skin_name}' AND pid = {$pid} ";
